...
function get_course_list($course){
  ...
  $url = "http://172.26.142.75:6060/Course/download/".$course.".txt";//current sem
  $data = rtrim(@file_get_contents($url));
  if(!url_exist($data)) return null;
  $data = explode("\n",$data);
  //print_r($data);
  array_shift($data);
  array_shift($data);
...
function visitor(){
  ...
  $lines = 7082 + count(file("searcher.txt"));//7082 search on previous year file
  fclose($file);
  return $lines;
}
...
function user_profile($roll_no){
  ...
  if($_SERVER['REMOTE_ADDR']=='172.24.0.119'){
